...
class AgentPiro:
    ...
    def process(self, user_input: str) -> str:
        ...
        # Incorporamos el system prompt dentro del primer user message.
        messages = []
        if not history:
            messages.append({
                "role": "user",
        ...
        while iteration < max_iter:
            iteration += 1
            logger.debug(f"Iteración {iteration}/{max_iter}")
        ...
        while iteration < max_iter:
            ...
            # Guardar fuente de la respuesta
            self.last_source = response.source
            ...
            # Si hay contenido en la respuesta, devolverla
            if response.content:
                # Guardar ambos mensajes juntos para mantener consistencia
                self.memory.add_message("user", user_input)
                self.memory.add_message("assistant", response.content)
                logger.agent_action("RESPONSE", response.content[:100])
                return response.content
